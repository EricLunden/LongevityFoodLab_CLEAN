...
// MARK: - Lightweight VM to cache filtered/sorted recipes (local to this file)
final class RecipesViewModel: ObservableObject {
    @Published var displayed: [Recipe] = []
    @Published var isLoaded = false
    private var lastComputeKey: String = ""

    func refresh(recipes: [Recipe], filter: FilterOption) {
        // Recompute only when inputs change
        // Include recipe IDs, ratings, and longevity scores in hash to detect when recipe properties change
        let recipeHash = recipes.map { "\($0.id.uuidString):\($0.rating):\($0.longevityScore ?? -1)" }.joined()
        let key = "\(recipes.count)|\(filter.rawValue)|\(recipeHash.hashValue)"
        guard key != lastComputeKey else { return }
        lastComputeKey = key

        DispatchQueue.global(qos: .userInitiated).async {
            let all = recipes.filter { !$0.isOriginal }
            ...
            var out = all
            switch filter {
            case .allRecipes, .mostRecent:
                out = out.sorted { $0.dateAdded > $1.dateAdded }
            case .topRated:
                out = out.filter { $0.rating > 0 }
                out = out.sorted { $0.rating > $1.rating }
            case .topIndexScores:
                out = out.filter { $0.longevityScore != nil }
                out = out.sorted { ($0.longevityScore ?? 0) > ($1.longevityScore ?? 0) }
            }
...
struct RecipesView: View {
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ScrollView {
                    VStack(spacing: 0) {
                        // Logo Header (matching Tracker screen)
                        logoHeaderSection
                        
                        // Recipes Top Box (with Edit and + buttons inside)
                        recipesTopBox
            ...
            .overlay(
                ...
                .onAppear {
                    if !hasInitialized {
                        ...
                        Task {
                            ...
                            // Set default view mode based on recipe count
                            await MainActor.run {
                                let recipeCount = recipeManager.recipes.filter { !$0.isOriginal }.count
                                if recipeCount > 6 {
                                    viewMode = .grid
                                } else {
                                    viewMode = .list
                            ...
                            vm.refresh(recipes: recipeManager.recipes, filter: filterOption)
                        }
                    } else {
                        // Reload recipes when returning to view to ensure latest scores are shown
                        Task {
                            await recipeManager.loadRecipes()
                            await MainActor.run {
                                vm.refresh(recipes: recipeManager.recipes, filter: filterOption)
                ...
                .onChange(of: recipeManager.recipes) { _, newRecipes in
                    vm.refresh(recipes: newRecipes, filter: filterOption)
                }
                .onChange(of: filterOption) { _, newFilter in
                    vm.refresh(recipes: recipeManager.recipes, filter: newFilter)
                }
        }
        .sheet(isPresented: $showingRecipeSearch) {
            SimpleRecipeSearchView { query in
                searchQuery = query
            }
    ...
    // MARK: - Edit and Add Buttons Section
    private var editAndAddButtonsSection: some View {
        HStack {
            // Edit/Cancel/Delete Button (left) - show in both grid and list views
            Button(action: {
            ...
            // Search Recipes Button (center) - moved from bottom
            Button(action: {
        ...
        .confirmationDialog("Delete Recipes", isPresented: $showingDeleteConfirmation) {
            ...
            Text("Are you sure you want to delete \(selectedRecipeIDs.count) recipe\(selectedRecipeIDs.count == 1 ? "" : "s")?")
        }
    }
    
    // MARK: - Button Text Computed Property
    ...
    // MARK: - Recipes Top Box
    private var recipesTopBox: some View {
        VStack(spacing: 16) {
            // Title with Icon (centered)
            HStack(spacing: 16) {
                // Food Icon with Gradient (left of title) - matching Type It gradient
            ...
            // Edit, All Recipes dropdown, and + Buttons (inside box at bottom)
            editAndAddButtonsSection
                .padding(.horizontal, 10)
        }
        .frame(maxWidth: .infinity)
        .padding(.top, 16)
    ...
    private var viewToggleSection: some View {
        HStack {
            ...
            // All Recipes Dropdown (center) - moved from top box
            Menu {
    ...
    // MARK: - Filtered Recipes (View-Layer Search)
    private var filteredRecipes: [Recipe] {
        guard !searchQuery.isEmpty else {
            return vm.displayed
        }
        let lowerQuery = searchQuery.lowercased()
        return vm.displayed.filter { $0.title.lowercased().contains(lowerQuery) }
    }
    ...
    private var recipeContentSection: some View {
        Group {
            if !vm.isLoaded {
                AnyView(ProgressView().padding(.top, 24))
            } else if filteredRecipes.isEmpty {
                AnyView(
                    VStack(spacing: 20) {
                        Image(systemName: searchQuery.isEmpty ? "book.fill" : "magnifyingglass")
                            .font(.system(size: 60))
                            .foregroundColor(.secondary)
                        
                        Text(searchQuery.isEmpty ? "No Recipes" : "No Search Results")
                            .font(.title3)
                            .fontWeight(.medium)
                            .foregroundColor(.primary)
                        
                        Text(searchQuery.isEmpty ? "Import or create a recipe to get started." : "Try a different search term.")
                            .font(.body)
                            .foregroundColor(.secondary)
                ...
                AnyView(recipeGridView(recipes: filteredRecipes))
            } else {
                AnyView(recipeListView(recipes: filteredRecipes))
            }
        }
    }
    ...
    private var bottomMenuBar: some View {
        HStack(spacing: 20) {
            // Search Recipes or Clear Search Button (left)
    ...
    private func getFilteredAndSortedRecipes() -> [Recipe] {
        var recipes = recipeManager.recipes.filter { !$0.isOriginal }
    ...
    private func toggleFavorite() {
        Task {
            ...
            do {
                ...
                print("RecipeDetailView: Failed to toggle favorite: \(error)")
                // Revert on error
                currentRecipe.isFavorite = recipe.isFavorite
            }
        }
    }
            ...
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    ...
                    if isTimeout {
                        ...
                        // For other errors, also show alert
                        showErrorAlert = true
                    }
                }
            }
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                // Background WITHOUT ignoresSafeArea
            ...
            .alert("Oops! Try Again?", isPresented: $showErrorAlert) {
                ...
                Button("No", role: .cancel) {
                    // User chose not to retry - do nothing
                }
            } message: {
...
struct RecipeGridCard: View {
    ...
    init(recipe: Recipe, isEditing: Bool, isSelected: Bool, onTap: @escaping () -> Void, onToggleSelection: @escaping () -> Void, scoreCircleSize: CGFloat = 40) {
        self.recipe = recipe
        ...
        Button(action: {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                GeometryReader { geometry in
                    ZStack(alignment: .bottomTrailing) {
                        // Recipe Image - ensure perfect square (using cached image system like Shop screen)
                        Group {
    ...
    // MARK: - Image Loading (removed - using AsyncImage directly now)
...
// MARK: - Simple Recipe Search View (Popup)
struct SimpleRecipeSearchView: View {
